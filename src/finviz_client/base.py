...
class FinvizClient:
    ...
    def _convert_volume_to_finviz_format(self, volume_value: Any) -> str:
        ...
        try:
            ...
            if isinstance(volume_value, str):
                if volume_value.startswith(("o", "u", "e")) or volume_value in [
                    "",
                    "frange",
                ]:
                    return volume_value
    ...
    def _convert_filters_to_finviz(self, filters: Dict[str, Any]) -> Dict[str, str]:
        ...
        # uptrend_screenerの場合の特別処理（正しい順序で生成）
            ...
            if "market_cap" in filters and filters["market_cap"]:
                ...
                if market_cap_val in cap_mapping:
                    params["f"] = (
                        params.get("f", "") + f"cap_{cap_mapping[market_cap_val]},"
                    )
                else:
            ...
            if volume_min is not None or volume_max is not None:
                volume_min_val = (
                    self._safe_numeric_conversion(volume_min)
                    if volume_min is not None
                    else None
                )
                volume_max_val = (
                    self._safe_numeric_conversion(volume_max)
                ...
                if volume_min_val and volume_min_val.startswith(("o", "u")):
                    ...
                    params["f"] = (
                        params.get("f", "")
                        + f"sh_volume_{volume_min_val}to{volume_max_val},"
                    )
                elif volume_min_val:
            ...
            if avg_volume_min is not None or avg_volume_max is not None:
                ...
                avg_vol_min_val = (
                    self._convert_volume_to_finviz_format(avg_volume_min)
                    if avg_volume_min is not None
                    else None
                )
                avg_vol_max_val = (
                    self._convert_volume_to_finviz_format(avg_volume_max)
                    if avg_volume_max is not None
                    else None
                )
                ...
                if avg_vol_min_val and avg_vol_min_val.startswith(("o", "u")):
                    ...
                    params["f"] = (
                        params.get("f", "")
                        + f"sh_avgvol_{avg_vol_min_val}to{avg_vol_max_val},"
                    )
                elif avg_vol_min_val:
                    # 下限のみ: sh_avgvol_100to
                    params["f"] = (
                        params.get("f", "") + f"sh_avgvol_{avg_vol_min_val}to,"
                    )
                elif avg_vol_max_val:
                    # 上限のみ: sh_avgvol_to500
                    params["f"] = (
                        params.get("f", "") + f"sh_avgvol_to{avg_vol_max_val},"
                    )
        ...
        if pe_min is not None or pe_max is not None:
            pe_min_val = (
                self._safe_numeric_conversion(pe_min) if pe_min is not None else None
            )
            pe_max_val = (
                self._safe_numeric_conversion(pe_max) if pe_max is not None else None
            )
        ...
        if "earnings_date" in filters and filters["earnings_date"]:
            ...
            # 従来の固定期間指定の場合
                ...
                if (
                    ...
                    params["f"] = (
                        params.get("f", "")
                        + f"earningsdate_{earnings_values[earnings_date_value]},"
                    )
                # リストの場合（複数条件のOR）
        ...
        # 売上前四半期比成長率フィルタ
        if (
            "sales_growth_qoq_min" in filters
            and filters["sales_growth_qoq_min"] is not None
        ):
