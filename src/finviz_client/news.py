...
class FinvizNewsClient(FinvizClient):
    ...
    def get_stock_news(
        self, tickers: Union[str, List[str]], days_back: int = 7, news_type: str = "all"
    ) -> List[NewsData]:
        """
    ...
    def _categorize_news(self, title: str) -> str:
        ...
        if any(
            word in title_lower
            for word in ["earnings", "revenue", "profit", "eps", "guidance"]
        ):
            return "earnings"
        elif any(
            word in title_lower
            for word in ["upgrade", "downgrade", "rating", "analyst", "target"]
        ):
            return "analyst"
        elif any(
            word in title_lower
            for word in ["insider", "ceo", "cfo", "director", "executive"]
        ):
            return "insider"
        elif any(
            word in title_lower for word in ["merger", "acquisition", "deal", "buyout"]
        ):
            return "merger"
        elif any(
            word in title_lower for word in ["fda", "approval", "clinical", "trial"]
        ):
            return "regulatory"
    ...
    def _parse_news_from_csv(
        self, row: "pd.Series", ticker: str, cutoff_date: datetime
    ) -> Optional[NewsData]:
        """
