...
def format_stock_data_table(
    stocks: List[StockData], fields: Optional[List[str]] = None
) -> str:
    """
    株式データをテーブル形式でフォーマット
...
def format_earnings_summary(stocks: List[StockData]) -> str:
    ...
    summary_lines = [f"Earnings Summary ({len(stocks)} stocks):", "=" * 50, ""]
...
def format_news_summary(news_list: List[NewsData]) -> str:
    ...
    summary_lines = [f"News Summary ({len(news_list)} articles):", "=" * 50, ""]
...
def format_screening_result_summary(
    stocks: List[StockData], params: Dict[str, Any]
) -> str:
    """
    ...
    if stocks:
        ...
        if volumes:
            summary_lines.append(
                f"  Average volume: {format_large_number(sum(volumes)/len(volumes))}"
            )
