...
def validate_price_range(
    min_price: Optional[Union[int, float, str]],
    max_price: Optional[Union[int, float, str]],
) -> bool:
    """
    価格範囲の妥当性をチェック
...
def validate_percentage(
    value: float, min_val: float = -100, max_val: float = 1000
) -> bool:
    """
    パーセンテージ値の妥当性をチェック
...
def validate_screening_params(params: Dict[str, Any]) -> List[str]:
    ...
    numeric_range_params = [
        ...
        "price_change_min",
        "price_change_max",
        "performance_week_min",
        "performance_month_min",
        "performance_quarter_min",
        "performance_halfyear_min",
        "performance_year_min",
        "performance_ytd_min",
        "volatility_week_min",
        "volatility_month_min",
        "week52_high_distance_min",
        "week52_low_distance_min",
        "eps_growth_this_year_min",
        "eps_growth_next_year_min",
        "eps_growth_past_5_years_min",
        "eps_growth_next_5_years_min",
        "sales_growth_quarter_min",
        "sales_growth_past_5_years_min",
        "insider_ownership_min",
        "insider_ownership_max",
        "institutional_ownership_min",
...
def validate_custom_range(
    param_name: str, min_val: Optional[float], max_val: Optional[float]
) -> bool:
    """
...
def validate_parameter_combination(params: Dict[str, Any]) -> List[str]:
    ...
    volume_count = sum(
        1 for v in volume_filters if v in params and params[v] is not None
    )
    if volume_count > 1:
    ...
    rel_volume_count = sum(
        1 for rv in rel_volume_filters if rv in params and params[rv] is not None
    )
    if rel_volume_count > 1:
        errors.append(
            "Use either relative_volume filter OR relative_volume_min, not both"
        )
...
def validate_and_normalize_raw_filters(raw_filters: str) -> Tuple[List[str], str]:
    ...
    for token in valid_tokens:
        if not token_pattern.match(token):
            errors.append(
                f"Invalid filter token: '{token}' (only lowercase a-z, 0-9, _, ., -, | allowed)"
            )
...
def validate_raw_sort_order(order: str) -> List[str]:
    ...
    if not re.match(r"^-?[a-z0-9_]+$", order):
        errors.append(
            f"Invalid sort order: '{order}' (only lowercase a-z, 0-9, _, optional leading '-')"
        )
    return errors
...
def validate_signal(signal: str) -> List[str]:
    ...
    if not re.match(r"^[a-z0-9_]+$", signal):
        errors.append(
            f"Invalid signal: '{signal}' (only lowercase a-z, 0-9, _ allowed)"
        )
    return errors
